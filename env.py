...
class Environment:
    def __init__(self, chain_length,chain_list,protein_name,thresh,pair_thres,metro_thres,log,path,epsln,learning_rate,outpath,noiter,nip,ip,term_thres,unbound,ds,f_cla,f_o):
    ...
    def load_protein_class(self):
        prot = Protein(self.no_chains,self.chain_list,self.protein_name,self.clash_threshold,self.pair_clash_threshold,self.metro_thres,log=self.logger,path=self.path,lr=self.lr,eps=self.eps,outpath=self.outpath,term_thres=self.term_state_thres,dsize=self.ds,unbound=self.unbound,f_cla=self.fc,f_o=self.fo)
        self.protein = prot
    ...
    def step(self, action,episode):
        ...
        if self.is_complex():
            ...
            # check if the final chain is problematic
            ...
            if clash_check and (self.repeat_track <5):
                ...
                const_status = False #self.protein.validate_progress_with_contrainst(self.model_status,self.action_list,self.interacting_pair,self.non_interacting_pair)
            ...
            #clash_check = self.protein.validate_progress_with_contrainst(self.model_status,self.action_list,self.interacting_pair,self.non_interacting_pair) #self.protein.validate_progress(self.model_status,self.action_list)
            clash_check = self.protein.validate_progress(self.model_status,self.action_list)
            #clash_check = False
            if clash_check and (self.repeat_track <5):
                #print('High clash loop at episode : ',episode, ' current repeat : ', self.repeat_track)
                #print('new action : ',action)
                #print('old action : ',self.old_action)
    ...
    def expand_tree(self):
        ...
        for x in list_of_possible_child:
            if x not in self.current_tree.children and self.will_not_cause_forest(self.current_tree.name,x) and self.child_is_interacting_with_parent(self.current_tree.name,x):
                # print(self.current_tree.name)
                # print(list_of_possible_child)
                # print('about to evaluate this child :', x)
                # print('accepting this child : ', x)
    ...
    def select_best_child(self):
        ...
        if len(self.current_tree.children) == 0: 
            ...
            self.expand_tree()
            # print('Randomly selecting a child since all have same value')
            # print(self.current_tree.name)
            # print(self.current_tree.children)
        ...
        if np.random.uniform() < self.eps_scalar:
            ...
            if len(bestchildren)==0:
                ...
                # probablyint("Returning random child")
                bc = np.random.choice(self.current_tree.children)

        # print(self.current_tree)
        # print(self.current_tree.children)
        # print('Value of all children : ',c_score_list)
        bc = np.random.choice(bestchildren)
        # print(bc)
        #print('Best child to return is : ',bc.name, ' with value : ', bc.reward)
        return bc #np.random.choice(bestchildren)
        #child_index = np.random.choice([x for x in range(0,len(children_list))])
        #return children_list[child_index]
    ...
    def build_children(self):
        ...
        for x in remaining_pairs:
            ...
            if len(list(set([item for sublist in built_pairs for item in sublist]))) > len(list(set([item for sublist in self.model_status for item in sublist]))):
                good_pairs.append(x)
        #if len(good_pairs) < len(remaining_pairs):
        #    print('Total number of remaining_child before pair filtering is ', len(remaining_pairs))
        #    print('Total number of remaining_child after pair filtering is ', len(good_pairs))
        #    print(self.model_status)
        #   print()
