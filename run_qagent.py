...
def update(no_of_episodes,qtable_path,qtable_out,log):
    ...
    for episode in range(no_of_episodes):
        ...
        while True:
            ...
            if repeat:
                action = RL.choose_action(observation) #RL.get_top5_actions(observation)#RL.get_top5_actions(str(observation))
            else:
                action = RL.choose_action(observation) # selects best decoy or random decoy base on chance
...
if __name__ == "__main__":
    ...
    output_dir = data_dir + '/data/mcrl_v2/' + protein + unbound_path + '/' + out_dir + '/'
